...
const Button=({sm=false,gray=false,className="",...Props}):ButtonProps & JSX.Element=>{
    ...
    return (
        <button className={`rounded-full transition-colors duration-300 disabled:cursor-not-allowed disabled:opacity-50 text-whitte ${sizeClass} ${colorClass} ${className}`}{...Props}>
        </button>
    )
}
